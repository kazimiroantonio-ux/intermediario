...
import { COMMISSION_STATUS_LABEL, formatCentsToKz } from "@/components/agent-labels";
...
export default function ComissoesPage() {
  ...
  const url = query ? `/api/agente/comissoes?${query}` : "/api/agente/comissoes";
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-200 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500">
                  ...
                  <th className="px-2 py-2">Valor do negócio</th>
                  <th className="px-2 py-2">Comissão</th>
                  <th className="px-2 py-2">Sua parte</th>
              ...
              <tbody className="divide-y divide-zinc-100">
                {data.items.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td className="px-2 py-2 text-zinc-600">{formatCentsToKz(c.dealAmountMinor)}</td>
                    <td className="px-2 py-2 text-zinc-600">
                      {formatCentsToKz(c.totalCommissionMinor)}
                      <span className="ml-1 text-xs text-zinc-400">{((c.rateBps / 100)).toFixed(0)}%</span>
                    </td>
                    <td className="px-2 py-2 font-semibold text-zinc-800">{formatCentsToKz(c.agentAmountMinor)}</td>
                    <td className="px-2 py-2">
                      <StatusBadge status={c.status} label={COMMISSION_STATUS_LABEL[c.status] ?? c.status} />
                    </td>
                    <td className="px-2 py-2 text-zinc-500">{shortDate(c.createdAt)}</td>
                    <td className="px-2 py-2 text-zinc-500">{shortDate(c.paidAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
